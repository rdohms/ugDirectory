...
require_once('polygon.vertex.class.php');	// A polygon consists of vertices. So the polygon
...
*/
	...
	*/
	...
	{
		$PT = 0.99999;			// Perturbation factor
		...
		elseif ($aP == 1)	// Move vertex p2 closer to p1
		{
			$p2->setX($p1->X() + $PT * ($p2->X() - $p1->X()));
			$p2->setY($p1->Y() + $PT * ($p2->Y() - $p1->Y()));
		}
		elseif ($aQ == 0)	// Move vertex q1 closer to q2
		{
			$q1->setX($q1->X() + (1-$PT) * ($q2->X() - $q1->X()));
			$q1->setY($q1->Y() + (1-$PT) * ($q2->Y() - $q1->Y()));
		}
		elseif ($aQ == 1)	// Move vertex q2 closer to q1
		{
	...
	*/
	function ints (&$p1, &$p2, &$q1, &$q2, &$n, &$ix, &$iy, &$alphaP, &$alphaQ)
	{
		...
		*/
		...
		// line segment.
		// A value less tha 0 or greater than 1 means teh intersection occurred
	...
	*/
	...
	{
		$winding_number = 0;
		$point_at_infinity =& new vertex(-infinity,$v->Y());	// Create point at infinity
	...
	*/
	function isPolyInside (&$p)
	{
		...
		{
			$c =& $p->getFirst();		// Get the first vertex in polygon $p
			$s =& $this->getFirst();	// Get the first vertex in thsi polygon
			do
			{
				do
				{
